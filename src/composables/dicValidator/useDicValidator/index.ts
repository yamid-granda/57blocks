...
import type { ComputedVariable, ReactiveVariable, ToRawRefs } from 'vue/macros'
import type { ValidationConfig, ValidationFunc, ValidationInfo, ValidationsDic } from '../types'

type UseDicValidator<Form> = ToRawRefs<{
  validationInfo: ValidationInfo<Form>
  isValid: ComputedVariable<boolean>
  validate: () => void
}>
...
export function useDicValidator<Dic>(
  ...
  const isValid = $computed<boolean>(() => {
    return Object
      .entries<ValidationConfig>(validationInfo)
      .every(([key, error]) => {
        if (!changedAttrsDic.value[key])
          return false
